# chinese-holiday-calendar-cli

boru-driven command-line client **and** interactive REPL for the ChineseHolidayCalendar
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chinese-holiday-calendar-cli)
make build

# 2. See usage (words, entities, env vars)
./chinese-holiday-calendar-cli --help

# 3. Provide credentials once, via the environment
export CHINESE_HOLIDAY_CALENDAR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./chinese-holiday-calendar-cli load 1 holiday            # {id:1} shorthand
./chinese-holiday-calendar-cli load '{id:1}' holiday       # explicit match map

# 5. Override the API base URL for a single call
CHINESE_HOLIDAY_CALENDAR_BASE=https://api.example.com ./chinese-holiday-calendar-cli load 1 holiday

# 6. No arguments -> interactive REPL
./chinese-holiday-calendar-cli
chinese-holiday-calendar> load 1 holiday
chinese-holiday-calendar> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chinese-holiday-calendar-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/chinese-holiday-calendar-cli load 1 holiday
   ```
...
```sh
./chinese-holiday-calendar-cli load 1 holiday          # scalar shorthand for {id:1}
./chinese-holiday-calendar-cli load '{id:1}' holiday     # explicit match map
```
...
./chinese-holiday-calendar-cli load 1 holiday
```
...
Run with no arguments to open a REPL (prompt `chinese-holiday-calendar>`). Each line is
evaluated as its own boru expression:

```text
$ ./chinese-holiday-calendar-cli
chinese-holiday-calendar> load 1 holiday
chinese-holiday-calendar> /help
chinese-holiday-calendar> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `holiday`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/chinese-holiday-calendar-cli`. |
...
(`./chinese-holiday-calendar-cli <expr>`) and interactively (the REPL), and expressions compose the
