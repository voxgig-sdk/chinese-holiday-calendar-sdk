// Command ChineseHolidayCalendar-cli is a CLI + REPL for the ChineseHolidayCalendar SDK,
...
const prompt = "chinese-holiday-calendar"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "holiday"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHINESE_HOLIDAY_CALENDAR_APIKEY (api key), CHINESE_HOLIDAY_CALENDAR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHINESE_HOLIDAY_CALENDAR_APIKEY carries the API key and
	// CHINESE_HOLIDAY_CALENDAR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHINESE_HOLIDAY_CALENDAR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHINESE_HOLIDAY_CALENDAR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewChineseHolidayCalendarSDK(opts)
