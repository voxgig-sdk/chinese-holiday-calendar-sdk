...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CHINESE_HOLIDAY_CALENDAR_TEST_LIVE") == "TRUE" ||
		os.Getenv("CHINESE_HOLIDAY_CALENDAR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CHINESE_HOLIDAY_CALENDAR_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.ChineseHolidayCalendarSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ChineseHolidayCalendarSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ChineseHolidayCalendarError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ChineseHolidayCalendarSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ChineseHolidayCalendarError{Msg: msg, Code: code}
}
