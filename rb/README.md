...
The Ruby SDK for the ChineseHolidayCalendar API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Holiday(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ChineseHolidayCalendar(nil).load(
  { "id" => "test01" }, nil
)
