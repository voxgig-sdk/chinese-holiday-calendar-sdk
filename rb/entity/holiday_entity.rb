# ChineseHolidayCalendar SDK Holiday entity
...
class HolidayEntity
  ...
  def data_set(args)
    if args
      @_data = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Holiday, Hash] the loaded Holiday; raises ChineseHolidayCalendarError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
