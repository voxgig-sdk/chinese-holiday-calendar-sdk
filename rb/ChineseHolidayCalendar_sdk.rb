...
require_relative 'ChineseHolidayCalendar_types'


class ChineseHolidayCalendarSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ChineseHolidayCalendarHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ChineseHolidayCalendarFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ChineseHolidayCalendarUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ChineseHolidayCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ChineseHolidayCalendarError.new(
        "#{op}_allow",
        "ChineseHolidayCalendarSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ChineseHolidayCalendarError => err
    ...
    if fetched.is_a?(Hash)
      status = ChineseHolidayCalendarHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ChineseHolidayCalendarError.new(
        "graphql_error", "ChineseHolidayCalendarSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ChineseHolidayCalendarSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
