...
module ChineseHolidayCalendarUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ChineseHolidayCalendarContext.new({}, nil)
    end
    op = ctx.op || ChineseHolidayCalendarOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ChineseHolidayCalendarResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ChineseHolidayCalendarError) ? err.msg : err.to_s
    ...
    sdk_err = ChineseHolidayCalendarError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ChineseHolidayCalendarError)
