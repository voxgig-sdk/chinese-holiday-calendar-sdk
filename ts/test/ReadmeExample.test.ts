...
describe('README example', () => {
  it('lead-language quickstart runs in test mode', async () => {
    ...
    const code = transformForTestMode(block, 'ChineseHolidayCalendar')

    // Run the (transformed) example. Async, so wrap in AsyncFunction.
    const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor
    const silentConsole = { log: () => {}, error: () => {}, warn: () => {} }
    const runner = new AsyncFunction('ChineseHolidayCalendarSDK', 'console', code)
    ...
    try {
      await runner(ChineseHolidayCalendarSDK, silentConsole)
    } catch (err: any) {
