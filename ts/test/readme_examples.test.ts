...
//     (\`new ChineseHolidayCalendarSDK(...)\` / \`ChineseHolidayCalendarSDK.test(...)\` become
//     \`ChineseHolidayCalendarSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ChineseHolidayCalendarSDK'
...
const TEST_SEED = {"entity":{"holiday":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ChineseHolidayCalendarSDK(...)\` and any
// \`ChineseHolidayCalendarSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ChineseHolidayCalendarSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ChineseHolidayCalendarSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
