...
class TestHolidayEntity:
    ...
    def test_should_create_instance(self):
        testsdk = ChineseHolidayCalendarSDK.test(None, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CHINESE_HOLIDAY_CALENDAR_TEST_HOLIDAY_ENTID JSON to run live")
        client = setup["client"]
...
def _holiday_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CHINESE_HOLIDAY_CALENDAR_TEST_HOLIDAY_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("CHINESE_HOLIDAY_CALENDAR_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = ChineseHolidayCalendarSDK(helpers.to_map(merged_opts))

    _live = env.get("CHINESE_HOLIDAY_CALENDAR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CHINESE_HOLIDAY_CALENDAR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
