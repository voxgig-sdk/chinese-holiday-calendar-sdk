...
The Python SDK for the ChineseHolidayCalendar API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Holiday(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ChineseHolidayCalendar(None).load(
    {"id": "test01"}, None
)
