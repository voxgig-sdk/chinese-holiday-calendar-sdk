...
from chineseholidaycalendar_sdk.core.helpers import get_ctx_prop, to_map
...
class ChineseHolidayCalendarContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ChineseHolidayCalendarControl()
        ...
        if isinstance(sp, ChineseHolidayCalendarSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ChineseHolidayCalendarResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ChineseHolidayCalendarResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ChineseHolidayCalendarOperation({})
    ...
    def make_error(self, code, msg):
        return ChineseHolidayCalendarError(code, msg, self)
