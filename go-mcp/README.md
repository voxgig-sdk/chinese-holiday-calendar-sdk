...
[MCP](https://modelcontextprotocol.io) server exposing the ChineseHolidayCalendar SDK as
two agent tools — `chinese-holiday-calendar_list` and `chinese-holiday-calendar_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chinese-holiday-calendar-mcp)
make build

# 2. Provide credentials via the environment
export CHINESE_HOLIDAY_CALENDAR_APIKEY=sk_live_xxx
...
```jsonc
// chinese-holiday-calendar_list: first page of records
{ "entity": "holiday" }
{ "entity": "holiday", "query": { } }

// chinese-holiday-calendar_load: one record by id
{ "entity": "holiday", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chinese-holiday-calendar-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user chinese-holiday-calendar \
     -- "$PWD"/dist/*/chinese-holiday-calendar-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `chinese-holiday-calendar_list` and `chinese-holiday-calendar_load` tools now appear
   in new sessions. Ask the agent to *"list holiday using chinese-holiday-calendar"*
   and it calls `chinese-holiday-calendar_list` with `{"entity":"holiday"}`.
...
```sh
export CHINESE_HOLIDAY_CALENDAR_APIKEY=sk_live_xxx            # API key
export CHINESE_HOLIDAY_CALENDAR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `chinese-holiday-calendar_list` tool
...
### Call the `chinese-holiday-calendar_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `chinese-holiday-calendar_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `chinese-holiday-calendar_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHINESE_HOLIDAY_CALENDAR_APIKEY` | API key sent with every request. |
| `CHINESE_HOLIDAY_CALENDAR_BASE` | Optional override of the API base URL. |
