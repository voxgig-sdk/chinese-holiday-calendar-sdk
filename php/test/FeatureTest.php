...
// ChineseHolidayCalendar SDK feature test
//
...
class FtCtrl extends ChineseHolidayCalendarControl
{
...
class FtClient
{
    ...
    private ChineseHolidayCalendarUtility $utility;

    public function __construct(ChineseHolidayCalendarUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChineseHolidayCalendarUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChineseHolidayCalendarContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ChineseHolidayCalendarContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ChineseHolidayCalendarContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChineseHolidayCalendarConfig::make_feature($name);
    ...
    private static function buildUrl(ChineseHolidayCalendarSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChineseHolidayCalendarError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChineseHolidayCalendarResponse($fetched);
        }
    ...
    private function fail(ChineseHolidayCalendarContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChineseHolidayCalendarContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
