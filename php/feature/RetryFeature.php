...
// ChineseHolidayCalendar SDK retry feature
...
class ChineseHolidayCalendarRetryFeature extends ChineseHolidayCalendarBaseFeature
{
    ...
    public function init(ChineseHolidayCalendarContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ChineseHolidayCalendarContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ChineseHolidayCalendarContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ChineseHolidayCalendarContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
