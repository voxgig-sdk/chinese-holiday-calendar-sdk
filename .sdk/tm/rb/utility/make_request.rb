...
module ChineseHolidayCalendarUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ChineseHolidayCalendarResponse.new({})
    ...
    if fetch_err
      ...
      response = ChineseHolidayCalendarResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ChineseHolidayCalendarResponse.new(fetched)
    else
