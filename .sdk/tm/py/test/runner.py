# ChineseHolidayCalendar SDK test runner
...
class ChineseHolidayCalendarTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ChineseHolidayCalendarTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ChineseHolidayCalendarTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ChineseHolidayCalendarTestRunner.getenv("PROJECTNAME_TEST_LIVE")
        override = ChineseHolidayCalendarTestRunner.getenv("PROJECTNAME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ChineseHolidayCalendarTestRunner.getenv(key)
        ...
        explain = ChineseHolidayCalendarTestRunner.getenv("PROJECTNAME_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ChineseHolidayCalendarTestRunner._test_control is not None:
            return ChineseHolidayCalendarTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ChineseHolidayCalendarTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ChineseHolidayCalendarTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ChineseHolidayCalendarTestRunner.load_test_control()
...
def load_env_local():
    ChineseHolidayCalendarTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return ChineseHolidayCalendarTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ChineseHolidayCalendarTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ChineseHolidayCalendarTestRunner.load_test_control()


def live_delay_ms():
    return ChineseHolidayCalendarTestRunner.live_delay_ms()
