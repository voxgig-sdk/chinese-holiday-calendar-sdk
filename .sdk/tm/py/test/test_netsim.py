# ChineseHolidayCalendar SDK netsim test
...
class TestNetsim:
    ...
    def test_offline_simulation_fails_the_request(self):
        sdk = ChineseHolidayCalendarSDK.test({"net": {"offline": True}})
    ...
    def test_fail_status_simulation_surfaces_the_error_status(self):
        sdk = ChineseHolidayCalendarSDK.test({"net": {"failTimes": 1, "failStatus": 503}})
        res = sdk.direct({"path": "/ping"})
    ...
    def test_error_times_simulation_yields_a_connection_error(self):
        sdk = ChineseHolidayCalendarSDK.test({"net": {"errorTimes": 1}})
        res = sdk.direct({"path": "/ping"})
        assert res["ok"] is False
    ...
    def test_latency_simulation_delays_the_request(self):
        delay = 60
        sdk = ChineseHolidayCalendarSDK.test({"net": {"latency": delay}})
        start = time.time()
    ...
    def test_injectable_sleep_keeps_latency_deterministic(self):
        slept = []
        sdk = ChineseHolidayCalendarSDK.test({"net": {"latency": 250,
                                           "sleep": slept.append}})
        sdk.direct({"path": "/ping"})
        assert slept == [250]

    def test_a_plain_test_sdk_still_works_with_no_net_simulation(self):
        sdk = ChineseHolidayCalendarSDK.test()
