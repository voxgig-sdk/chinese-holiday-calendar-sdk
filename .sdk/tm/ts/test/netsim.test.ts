...
describe('netsim', () => {
  ...
  test('offline simulation fails the request', async () => {
    const sdk = ChineseHolidayCalendarSDK.test({ net: { offline: true } })
  ...
  test('failStatus simulation surfaces the error status', async () => {
    const sdk = ChineseHolidayCalendarSDK.test({ net: { failTimes: 1, failStatus: 503 } })
    const res: any = await sdk.direct({ path: '/ping' })
    equal(res.ok, false)
  ...
  test('latency simulation delays the request', async () => {
    const delay = 60
    const sdk = ChineseHolidayCalendarSDK.test({ net: { latency: delay } })
  ...
  test('a plain test SDK still works with no net simulation', async () => {
    const sdk = ChineseHolidayCalendarSDK.test()
    equal(null !== sdk, true)
  })
})
