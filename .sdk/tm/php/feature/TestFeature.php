...
// ChineseHolidayCalendar SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ChineseHolidayCalendarTestFeature extends ChineseHolidayCalendarBaseFeature
{
    ...
    public function init(ChineseHolidayCalendarContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ChineseHolidayCalendarContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ChineseHolidayCalendarParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ChineseHolidayCalendarContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ChineseHolidayCalendarContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ChineseHolidayCalendarParam::call($ctx, $k);
